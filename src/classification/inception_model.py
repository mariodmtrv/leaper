...
class InceptionModel(LearningModel):
  ...
  def prepare_for_transfer_learning(self):
    ...
    output = Dense(activation='relu', units=30)(output)
    output = Dense(activation='sigmoid', units=30)(
        output)
    self.inception_model = Model(self.inception_model.input, output)
    # print(self.inception_model.summary())
    train_datagen = ImageDataGenerator(
        preprocessing_function=preprocess_input, rotation_range=40,
        zoom_range=[0.7, 0.9],
...
if __name__ == '__main__':
  ...
  MODEL_PATH = BASE_PATH + "/models" + "/inception_ml_vision_model.h5"
